...
def calcul_date_prod(lot):
    ...
    if lot_premier_caractere not in chiffres: 
        ...
        try:
            ...
            lot_un = datetime.strptime(quantieme_et_annee_auj, '%j/%Y') #%j = day of the year, on crée l'objet date
            
            if date_auj > lot_un:
                return lot_un.strftime('%d/%m/%Y') # on repasse en objet str avec le format voulu
